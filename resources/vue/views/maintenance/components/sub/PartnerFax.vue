  ...
  <div>
    <el-row :gutter="15">
      ...
      <el-col :span="10" :offset="7" class="fax-cls-mainCode">
        <el-row
          :gutter="15"
          style="border: 1px solid #000000; padding: 5px; margin-top: 10px"
        >
          <el-col :span="11"> 受付番号: </el-col>
          <el-col :span="13">
    ...
    <el-row :gutter="20">
      ...
      <el-col :span="14" align="middle">
        ...
        <el-row :gutter="15" style="font-size: 12px">
          <el-col :span="12"> TEL: {{ detail.TEL }} </el-col>
          <el-col :span="12"> FAX: {{ detail.FAX }} </el-col>
        </el-row>
      </el-col>
      ...
      <el-col :span="10">
        <el-row
          >請求先: {{ computedAccounting_Name(detail.accounting_info) }}</el-row
        >
        <el-row>
          <el-col :span="14" align="middle">
            <!-- {{ detail.user.name }} -->
    ...
    <el-row>
      ...
      <el-row>
        <button
          style="padding: 7px; background-color: white; border: 1px solid black"
        >
          尚、修理代が50,000円以上になる場合には連絡及びお見積りをお願いします。
        </button>
      </el-row>
    </el-row>
    ...
    <el-row style="border: 1px solid black; padding: 10px">
      ...
      <el-row :gutter="10" style="margin-right: 0px; margin-left: 0px">
        <table class="fax-detail-table">
          <tr>
            <td rowspan="2" style="width: 15%; border-right: 1px solid #000000">
              店舗結品先
            </td>
      ...
      <el-row :gutter="10" style="margin-right: 0px; margin-left: 0px">
        <table class="fax-detail-table">
          <tr>
            <td style="width: 15%; border-right: 1px solid #000000">住所</td>
            <td>
      ...
      <el-row :gutter="10" style="margin-right: 0px; margin-left: 0px">
        <table class="fax-detail-table">
          <tr>
            <td
              style="border-right: 1px solid #000000; width: 15%; height: 6em"
            >
              状況
            </td>
            <td>{{ detail.order }}</td>
    ...
    <el-row style="border: 1px solid black; padding: 10px">
      <el-row :gutter="10" style="margin-right: 0px; margin-left: 0px">
        <table class="fax-no-border-table">
          <tr>
            <td rowspan="2" style="font-size: 18px; font-weight: bold">
              お取引先様
            </td>
        ...
        <table class="fax-no-border-table">
          ...
          <tr>
            ...
            <td
              ...
              "
            >
              {{ computedvisitDate(detail.visit_schedule_date) }}
            </td>
          </tr>
        </table>
      </el-row>
      ...
      <el-row
        >※
        作業報告および写真の提出をお願いします。書類確認できない場合は、お支払いできないことがあります。</el-row
      >
      ...
      <el-row>
        <table class="fax-detail-table">
          <tr>
            ...
            <td style="border-right: 1px solid #000000; width: 35%">
              {{ detail.completed_date }}
            </td>
            <td style="border-right: 1px solid #000000; width: 15%">見積額</td>
            <td>
              <span
                v-if="detail.accounting_info && detail.accounting_info.length"
                >{{ detail.accounting_info[0].unincluding_price }}</span
              >
            </td>
          </tr>
        </table>
      ...
      <el-row>
        <table class="fax-detail-table">
          <tr>
            <td style="border-right: 1px solid #000000; width: 15%" rowspan="2">
              内容・経過
            </td>
            <td style="border-right: 1px solid #000000; width: 35%" rowspan="2">
              <span
                v-if="
                  detail.maintenance_progress &&
                  detail.maintenance_progress.length
                "
                >{{ detail.maintenance_progress[0].comment }}</span
              >
            </td>
...
.dash-dot-dot {
  height: 50px;
  background: radial-gradient(circle at center, #000 2px, transparent 3px) 0px
      50%/30px 5px repeat-x,
    radial-gradient(circle at center, #000 2px, transparent 3px) 10px 50%/30px
      5px repeat-x,
  ...
  methods: {
    computedvisitDate(date) {
      if (date != '' && date != null) {
        ...
        return (
          date_arr[0] + '/' + date_arr[1] + '/' + date_arr[2].split(' ')[0]
        );
      }
    },
